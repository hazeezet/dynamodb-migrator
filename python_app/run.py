...
def main():
    ...
    global_group.add_argument(
        ...
        metavar="<STATE_DIR>",
        default=".",
        help="Directory for state files (migration_state.json, undo_state.json) [default: .]",
    )
    global_group.add_argument(
        "--log-file",
        metavar="<LOG_FILE>",
    ...
    global_group.add_argument(
        "-h", "--help", action="help", help="Print help (see a summary with '-h')"
    )
    global_group.add_argument(
    ...
    migrate_options.add_argument(
        "-s",
        "--source",
        metavar="<SOURCE>",
        help="Source DynamoDB table name (non-interactive mode)",
    )
    migrate_options.add_argument(
        "-t",
        "--target",
        metavar="<TARGET>",
        help="Target DynamoDB table name (non-interactive mode)",
    )
    migrate_options.add_argument(
        "-m",
        "--mappings",
        metavar="<MAPPINGS>",
        help="Path to JSON mappings file (non-interactive mode)",
    )
    migrate_options.add_argument(
        "-p",
        "--passthrough",
        action="store_true",
        help="Use passthrough mode - copy all attributes (non-interactive mode)",
    )
    migrate_options.add_argument(
        "-e",
        "--exclude",
        metavar="<EXCLUDE>",
        help="Comma-separated columns to exclude in passthrough mode",
    )
    migrate_options.add_argument(
        "--state-dir",
        metavar="<STATE_DIR>",
        default=".",
        help="Directory for state files [default: .]",
    )
    migrate_options.add_argument(
        "--log-file",
        metavar="<LOG_FILE>",
    ...
    resume_parser.add_argument(
        "-i", "--id", help="Migration ID to resume (interactive if omitted)"
    )
    resume_parser.add_argument(
        "--state-dir", default=".", help="Directory for state files"
    )
    ...
    undo_parser.add_argument(
        "-i", "--id", help="Migration ID to undo (interactive if omitted)"
    )
    undo_parser.add_argument(
        "--state-dir", default=".", help="Directory for state files"
    )
    ...
    if args.command == "list":
        ...
        for m in state["migrations"]:
            print(
                f"ID: {m['id']} | {m['source_table']} -> {m['target_table']} | Status: {m['status']}"
            )
        return
    ...
    if args.command == "undo":
        if args.id:
            migration = next(
                (m for m in state["migrations"] if m["id"] == args.id), None
            )
            if not migration:
    ...
    if args.command == "migrate":
        if args.source and args.target:
            ...
            if args.passthrough:
                exclude_list = (
                    [c.strip() for c in args.exclude.split(",")] if args.exclude else []
                )
                ...
                print(
                    "Error: --passthrough or --mappings must be provided in non-interactive mode"
                )
                sys.exit(1)
